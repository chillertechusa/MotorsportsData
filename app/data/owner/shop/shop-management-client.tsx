...
const STATUS_PILL: Record<string, string> = {
  paid:            'bg-blue-600/10 text-blue-500 border border-blue-600/20',
      ...
      >
        <div className="min-w-0 flex-1 grid grid-cols-2 gap-x-4 gap-y-1 sm:grid-cols-4">
          ...
          <div>
            ...
            <p className="text-sm font-semibold text-green-500">{dollars(order.totalCents)}</p>
          </div>
          <div>
        ...
        <div className="border-t border-zinc-800 px-5 py-4 space-y-4">
          ...
          <div className="rounded border border-zinc-800 bg-zinc-950 px-4 py-3 text-sm space-y-1">
            ...
            <div className="flex justify-between font-bold text-zinc-100 border-t border-zinc-800 pt-1 mt-1">
              <span>Total</span><span className="text-green-500">{dollars(order.totalCents)}</span>
            </div>
          </div>
            ...
            <div className="space-y-2">
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 rounded border border-zinc-700 bg-zinc-950 px-3 py-2 text-sm text-zinc-100 placeholder:text-zinc-600 focus:border-green-500 focus:outline-none"
                />
                <input
              ...
              <div className="flex gap-2">
                <button
                  ...
                  disabled={isPending}
                  className="rounded bg-green-500 px-4 py-1.5 text-xs font-bold uppercase tracking-wider text-zinc-950 hover:bg-green-400 disabled:opacity-50"
                >
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1">
            {canAdvance && (
              <button
                ...
                disabled={isPending}
                className="flex items-center gap-1.5 rounded bg-green-500 px-4 py-2 text-xs font-bold uppercase tracking-wider text-zinc-950 hover:bg-green-400 disabled:opacity-50"
              >
      ...
      >
        <div className="min-w-0 flex-1 grid grid-cols-2 gap-x-4 gap-y-1 sm:grid-cols-4">
          ...
          <div>
            ...
            <p className={`text-sm font-semibold ${outOfStock ? 'text-red-400' : lowStock ? 'text-yellow-400' : 'text-green-500'}`}>
              {product.totalStock} units
            </p>
          </div>
          <div>
            <p className="text-[10px] font-semibold uppercase tracking-widest text-zinc-500">Status</p>
            <span className={`inline-block mt-0.5 rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase tracking-wide ${product.active ? 'bg-green-500/10 text-green-500 border border-green-500/20' : 'bg-zinc-800 text-zinc-500 border border-zinc-700'}`}>
                    ...
                    {editingVariant === v.id ? (
                      <>
                        <input
                          ...
                          className="w-20 rounded border border-green-500/50 bg-zinc-900 px-2 py-1 text-sm text-zinc-100 focus:outline-none"
                          autoFocus
                          onKeyDown={(e) => { if (e.key === 'Enter') saveStock(v.id); if (e.key === 'Escape') setEditingVariant(null) }}
                        />
                        <button onClick={() => saveStock(v.id)} disabled={isPending} className="text-xs text-green-500 hover:text-green-400 font-semibold">Save</button>
