        ...
        <div className="border-t border-zinc-800 px-5 py-4 grid grid-cols-1 sm:grid-cols-2 gap-2.5">
          {!loaded ? (
            ...
            <div key={label} className={`flex items-start gap-3 rounded-xl px-3.5 py-2.5 border ${filled ? 'border-zinc-700/50 bg-zinc-800/40' : 'border-zinc-800/50 bg-transparent opacity-50'}`}>
              <Icon className={`h-4 w-4 mt-0.5 shrink-0 ${filled ? 'text-green-500' : 'text-zinc-600'}`} />
        ...
        {messages.map((m, i) => (
          <div key={i} className={`flex ${m.role === 'user' ? 'justify-end' : 'justify-start'}`}>
            ...
            >
              {m.role === 'ai' ? (
                <>
                  <p className="text-xs font-bold uppercase tracking-wider text-green-500 mb-1.5">Race Coach</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
            {QUICK_PROMPTS.map((qp) => {
              ...
              return (
                ...
                >
                  <span className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-green-500/15 text-green-500">
